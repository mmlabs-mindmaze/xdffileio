...
int main(int argc, char *argv[])
{
	...
	snprintf(reffilename, sizeof(reffilename),
		 "%s/ref%u-%u-%u-%u-%u-%u-%u.gdf2", SRCDIR, SAMPLINGRATE, DURATION,
	...
	if (!retcode)
		retcode = test_validation_param(XDF_GDF2, numtype, 
						supported_type);

	if (!retcode)
		retcode = test_seek(XDF_GDF2);
