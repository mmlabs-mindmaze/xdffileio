 ...
 *
 * Return: 
 * an handle to XDF file opened in case of success.
 ...
 *
 * EINVAL
 *   @mode is neither XDF_READ nor XDF_WRITE, or if @filename is NULL
 */
API_EXPORTED
 ...
 *
 * EBADF
 *   the @fd argument is not a valid file descriptor or has been opened
 *   with flags incompatible with the @mode argument (O_WRONLY for XDF_READ
 *   or O_RDONLY for XDF_WRITE)
 */
API_EXPORTED
 ...
 *
 * Return: 
 * the handle to requested channel descriptor in case of success, 
 * NULL otherwise, and errno is set
 *
 * Errors:
 * EINVAL
 ...
 *
 * Return: 
 * the handle to newly created channel descriptor in case of success.
 * Otherwise NULL is returned and errno is set appropriately.
 *
 * Errors:
 * EINVAL
 ...
 *   specifies the array from/to which the channel 
 ...
 *
 * XDF_CF_PMAX (double) [max of \fBXDF_CF_ARRTYPE\fP]
 ...
 *   sets the minimal value that a digital value can get. Cannot be set if
 *   XDF_READ. This is also automatically set by XDF_CF_STOTYPE.
 *
 * XDF_CF_DMAX (double) [min of XDF_CF_STOTYPE]
 *   sets the maximal value that a digital value can get. Cannot be set if
 *   XDF_READ. This is also automatically set by XDF_CF_STOTYPE.
 *
 * XDF_CF_LABEL (const char*) [""]
 *   sets the label of the channel. Cannot be set if XDF_READ.
 *
 * XDF_CF_UNIT (const char*) [""] {EDF BDF GDF}
 *   sets the unit of the channel. Cannot be set if XDF_READ.
 *
 * XDF_CF_TRANSDUCTER (const char*) [""] {EDF BDF GDF}
 *   sets the type of sensor used for this channel. Cannot be set if XDF_READ.
 *
 ...
 *   sets the impedance of the sensor/electrode. Cannot be set if XDF_READ.
 *
 * Return:
 ...
 *
 * EPERM
 *   the request submitted is not allowed for this channel or is forbidden for
 *   file opened with the mode XDF_READ
 *
 * EDOM
 *   the value set in xdf_set_chconf() as digital or physical min/max (fields
 *   XDF_CF_{D/P}{MIN/MAX}) goes beyond the limits of respectively the stored
 *   or array data type).
 *
 *
 * Example:
 *    //Assume xdf referenced an XDF file opened for writing
 *    unsigned int array = 2, offset = 0;
 *    const char label[] = "Channel EEG";
 *
 *    hchxdf ch = xdf_add_channel(xdf);
 *    xdf_set_chconf(ch, XDF_CF_ARRINDEX, array, XDF_CF_LABEL, label, XDF_NOF);
 */
 ...
 *
 * XDF_CF_PMIN (double) [min of XDF_CF_ARRTYPE]
 *   gets the minimal value that a physical value can get.
 *
 * XDF_CF_PMAX (double) [max of \fBXDF_CF_ARRTYPE\fP]
 *   gets the maximal value that a physical value can get.
 *
 * XDF_CF_STOTYPE (enum xdftype) [any datatype supported by file type]
 *   specifies the type stored in the file of the channel value.
 *
 * XDF_CF_DMIN (double) [min of XDF_CF_STOTYPE]
 *   gets the minimal value that a digital value can get.
 *
 * XDF_CF_DMAX (double) [min of XDF_CF_STOTYPE]
 *   gets the maximal value that a digital value can get.
 *
 * XDF_CF_LABEL (const char*) [""]
 *   gets the label of the channel.
 *
 * XDF_CF_UNIT (const char*) [""] {EDF BDF GDF}
 *   gets the unit of the channel.
 *
 * XDF_CF_TRANSDUCTER (const char*) [""] {EDF BDF GDF}
 *   gets the type of sensor used for this channel.
 *
 * XDF_CF_PREFILTERING (const char*) [""] {EDF BDF GDF}
 *   gets the information about the filters already applied on channel data.
 *
 * XDF_CF_ELECPOS (double[3]) [0,0,0] {GDF}
 ...
 *
 * XDF_CF_IMPEDANCE (double) [0] {GDF}
 *   gets the impedance of the sensor/electrode.
 *
 * Return:
 * 0 in case of success. Otherwise -1 is returned and errno is set
 * appropriately.
 ...
 *    xdf_get_chconf(ch, XDF_CF_ARRINDEX, &iarray, XDF_CF_LABEL, &l, XDF_NOF);
 *
 *    printf("iarray = %u\\n", iarray);
 *    printf("offset = %u\\n", offset);
 *    printf("label = %s\\n", l);
 ...
 *   specifies the duration of one record. The value should be positive.
 *
 * XDF_F_REC_NSAMPLE (int) [0]
 *   specifies the number of time points contained
 ...
 *   sets the sampling frequency of the recording.
 ...
 *   sets patient classification
 *   according to the International Statistical Classification of Diseases and
 *   Related Health Problems (ICD).
 *
 * XDF_F_HEADSIZE (double[3]) [0,0,0] {GDF}
 *   sets size of the subject's head (circumference, distance nasion to inion,
 *   distance left to right mastoid)
 *   expressed in millimeters.
 *
 * BXDF_F_REF_POS (double[3]) [0,0,0] {GDF}
 *   sets X, Y, Z coordinates of the reference electrode.
 *
 * XDF_F_GND_POS (double[3]) [0,0,0] {GDF}
 *   sets X, Y, Z coordinates of the ground electrode.
 *
 * Return: 
 * 0 in case of success. Otherwise -1 is returned and errno is set
 * appropriately.
 ...
 * Errors:
 * EINVAL
 *   @xdf is NULL or @field is not a proper value of the enumeration xdffield
 *   accepted by the function (for example XDF_F_NCHANNEL)
 *
 * EPERM
 *   the request submitted to xdf_set_conf is not allowed for this type of XDF
 *   file or is not supported with the mode XDF_READ.
 *
 *
 * Example:
 ...
 *    const char * subj, * s;
 *
 *    xdf_get_conf(xdfr, XDF_F_SUBJ_DESC, &subj, XDF_F_SESS_DESC, &s, XDF_NOF);
 *
 *    xdf_set_conf(xdfw, XDF_F_SUBJ_DESC, subj, XDF_F_SESS_DESC, s, XDF_NOF);
 */
API_EXPORTED
 ...
 *
 * XDF_F_SAMPLING_FREQ (int) [0]
 *   gets the sampling frequency of the recording.
 *
 * XDF_F_NCHANNEL (int)
 *   gets the number of channel in the file.
 *
 * XDF_F_NEVTTYPE (int)
 *   gets the number of different event types.
 *
 * XDF_F_NEVENT (int)
 *   gets the number of events.
 *
 * XDF_F_NREC (int)
 *   gets the number of record in the file.
 *
 * XDF_F_FILEFMT (int)
 *   gets the file format type (one of the value defined
 *   by the enumeration xdffiletype other than XDF_ANY).
 *
 * XDF_F_RECTIME (double) [current time] {EDF BDF GDF}
 *   gets date and time of
 *   recording. It is expressed as number of seconds elapsed since the Epoch,
 *   1970-01-01 00:00:00 +0000 (UTC). This number is the conversion to double
 *   of the value returned by the function time() of the standard C library. On
 *   file creation, this field is initialized to the current time.
 ...
 *
 * XDF_F_BIRTHDAY (double) [0] {GDF}
 *   gets birthday of the patient using the
 *   same format as specified for field XDF_F_RECTIME.
 *
 * XDF_F_HEIGHT (double) [0] {GDF}
 *   gets height of the subject in centimeters.
 *
 * XDF_F_WEIGHT (double) [0] {GDF}
 *   gets weight of the subject in kilograms.
 *
 * XDF_F_GENDER (unsigned int) [0] {GDF}
 *   gets sex of the subject. Use 1 for male, 2 for female and 0 if unknown.
 *
 * XDF_F_HANDNESS (unsigned int) [0] {GDF}
 *   gets handness of the subject. Use
 *   0 if unknown, 1 if right, 2 if left and 3 if ambidextrious.
 *
 * XDF_F_VISUAL_IMP (unsigned int) [0] {GDF}
 *   gets visual impairment. Use 0
 *   if unknown, 1 if no impairment, 2 if impaired and 3 if impaired but
 *   corrected.
 *
 * XDF_F_HEART_IMP (unsigned int) [0] {GDF}
 *   gets heart impairment. Use 0 if
 *   unknown, 1 if no impairment, 2 if impaired and 3 if the subject wear a
 *   pacemaker.
 *
 * XDF_F_LOCATION (double[3]) [0,0,0] {GDF}
 *   gets location of the recording.
 *   The first 2 component specify the latitude and longitude in degrees, the
 *   third specifies the altitude in meters.
 *
 * XDF_F_ICD_CLASS (char[6]) [0x000000000000] {GDF}
 *   gets patient classification
 *   according to the International Statistical Classification of Diseases and
 *   Related Health Problems (ICD).
 *
 * XDF_F_HEADSIZE (double[3]) [0,0,0] {GDF}
 *   gets size of the subject's head
 *   (circumference, distance nasion to inion, distance left to right mastoid)
 *   expressed in millimeters.
 *
 * BXDF_F_REF_POS (double[3]) [0,0,0] {GDF}
 *   gets X, Y, Z coordinates of the reference electrode.
 *
 * XDF_F_GND_POS (double[3]) [0,0,0] {GDF}
 *   gets X, Y, Z coordinates of the ground electrode.
 *
 * Return: 
 * 0 in case of success. Otherwise -1 is returned and errno is set
 * appropriately.
 ...
 *
 * EPERM
 *   the request submitted is not supported with the mode XDF_READ
 *
 *
 * Example:
 ...
 *
 * Return: 
 * the selected data type in case of success, otherwise -1 and
 * errno is set appropriately
 *
 * Errors:
 * EINVAL
 *   the @xdf pointer is NULL, or if the argument @type is not an admissible
 *   enum xdftype value
 */
API_EXPORTED
 ...
 *
 * Return: 
 * the event type in case of success. Otherwise -1 is returned and 
 * errno is set appropriately.
 *
 * Errors:
 * EINVAL
 ...
 *
 * EPERM
 *   the file referenced by @xdf has not been opened for writing or its
 ...
 * Errors:
 * EINVAL
 *   @xdf is NULL or @evttype has not been previously created by
 *   xdf_add_evttype()
 *
 * ENOMEM
 *   the system is unable to allocate resources
 *
 * EPERM 
 *   the file referenced by @xdf has not been opened for writing or 
 *   its file format does not support events
 */
API_EXPORTED 
 ...
 * Errors:
 * EINVAL
 *   @xdf, @evttype, @start or @dur is NULL
 *
 * ERANGE
